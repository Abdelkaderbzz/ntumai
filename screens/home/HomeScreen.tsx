    ...
    <ScrollView className="flex-1 bg-gray-50">
      ...
      <View className="px-4 mb-6">
        ...
        <View className="flex-row flex-wrap gap-2">
          {thinkingCards.map((item, idx) => (
            <TouchableOpacity
              key={item.title}
              className={`mb-2 ${idx % 2 === 0 ? "mr-1" : "ml-1"}`}
              style={{ minWidth: cardWidth, flex: 1 }}
              activeOpacity={0.9}
            >
              <View
                className={`rounded-2xl h-24 shadow-sm overflow-hidden relative bg-black`}
              >
                <Image
                  source={item.image}
                  ...
                  resizeMode="cover"
                />
                {/* Colored overlay for extra tint, adjust as needed */}
                <View
                  className={`absolute inset-0 bg-black`}
                  style={{ opacity: 0.2 }}
                />
                <View className="flex-1 justify-end p-4 items-center">
                  <AppText
